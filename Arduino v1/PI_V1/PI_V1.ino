...
// PID related constants and variables, I'll put these in approrpiate places later
float thetaSet = 0;
float Kp = 50, Ki = .0003;
float integralE = 0;
...
void loop()
{
  ...
  if (time - lastActionTime >= LOOP_DURATION) {
    ...
    error = thetaSet - ypr[2];
    integralE = integralE + error*(time - lastActionTime);
    sum = Kp*error + Ki*integralE;
    power = - int ((510/(1 + exp(-sum))) - 255);

    Serial.println(Kp*error);
    Serial.println(Ki*integralE);
    Serial.print("power:\t"); Serial.println(power);
    
    // drive the motors with calculated power
    driveMotors(power);
